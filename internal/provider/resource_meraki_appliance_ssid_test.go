...
func TestAccMerakiApplianceSSID(t *testing.T) {
	...
	steps = append(steps, resource.TestStep{
		Config: testAccMerakiApplianceSSIDPrerequisitesConfig + testAccMerakiApplianceSSIDConfig_all(includeWriteOnly),
...
func testAccMerakiApplianceSSIDConfig_all(includeWriteOnly bool) string {
	config := `resource "meraki_appliance_ssid" "test" {` + "\n"
	config += `  network_id = meraki_network_device_claim.test.network_id` + "\n"
	config += `  number = "1"` + "\n"
