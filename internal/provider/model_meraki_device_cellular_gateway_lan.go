...
type DeviceCellularGatewayLAN struct {
	Id                 types.String                                 `tfsdk:"id"`
	Serial             types.String                                 `tfsdk:"serial"`
	FixedIpAssignments []DeviceCellularGatewayLANFixedIpAssignments `tfsdk:"fixed_ip_assignments"`
	ReservedIpRanges   []DeviceCellularGatewayLANReservedIpRanges   `tfsdk:"reserved_ip_ranges"`
}

type DeviceCellularGatewayLANFixedIpAssignments struct {
	Ip   types.String `tfsdk:"ip"`
	Mac  types.String `tfsdk:"mac"`
	Name types.String `tfsdk:"name"`
}

type DeviceCellularGatewayLANReservedIpRanges struct {
...
func (data DeviceCellularGatewayLAN) getPath() string {
...
func (data DeviceCellularGatewayLAN) toBody(ctx context.Context, state DeviceCellularGatewayLAN) string {
	body := ""
	{
...
func (data *DeviceCellularGatewayLAN) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("fixedIpAssignments"); value.Exists() && value.Value() != nil {
		data.FixedIpAssignments = make([]DeviceCellularGatewayLANFixedIpAssignments, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DeviceCellularGatewayLANFixedIpAssignments{}
	...
	if value := res.Get("reservedIpRanges"); value.Exists() && value.Value() != nil {
		data.ReservedIpRanges = make([]DeviceCellularGatewayLANReservedIpRanges, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DeviceCellularGatewayLANReservedIpRanges{}
...
func (data *DeviceCellularGatewayLAN) fromBodyPartial(ctx context.Context, res meraki.Res) {
...
func (data *DeviceCellularGatewayLAN) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
