...
func (data CameraDeviceWirelessProfiles) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("ids.backup"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ids.backup", "null")
		} else {
			body, _ = sjson.Set(body, "ids.backup", value.String())
		}
	}
	if value := res.Get("ids.primary"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ids.primary", "null")
		} else {
			body, _ = sjson.Set(body, "ids.primary", value.String())
		}
	}
	if value := res.Get("ids.secondary"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ids.secondary", "null")
		} else {
			body, _ = sjson.Set(body, "ids.secondary", value.String())
		}
	}
	return body
}
...
func (data CameraDeviceWirelessProfiles) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
