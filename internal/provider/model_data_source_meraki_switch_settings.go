...
// It emits a separate model - DataSourceSwitchSettings - used only by the data source, always including every
...
type DataSourceSwitchSettings struct {
	...
	PowerExceptions                []DataSourceSwitchSettingsPowerExceptions `tfsdk:"power_exceptions"`
}

type DataSourceSwitchSettingsPowerExceptions struct {
...
func (data DataSourceSwitchSettings) getPath() string {
...
func (data *DataSourceSwitchSettings) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("powerExceptions"); value.Exists() && value.Value() != nil {
		data.PowerExceptions = make([]DataSourceSwitchSettingsPowerExceptions, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceSwitchSettingsPowerExceptions{}
