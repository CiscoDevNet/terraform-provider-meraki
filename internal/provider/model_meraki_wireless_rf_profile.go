...
type WirelessRFProfileIdentity struct {
	NetworkId types.String `tfsdk:"network_id"`
	Id        types.String `tfsdk:"id"`
}
...
func (data *WirelessRFProfile) fromBodyPartial(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("isIndoorDefault"); value.Exists() && !data.IsIndoorDefault.IsNull() {
		data.IsIndoorDefault = types.BoolValue(value.Bool())
	} else {
		data.IsIndoorDefault = types.BoolNull()
	}
	if value := res.Get("isOutdoorDefault"); value.Exists() && !data.IsOutdoorDefault.IsNull() {
		data.IsOutdoorDefault = types.BoolValue(value.Bool())
	} else {
		data.IsOutdoorDefault = types.BoolNull()
	}
...
func (data *WirelessRFProfileIdentity) toIdentity(ctx context.Context, plan *WirelessRFProfile) {
...
func (data *WirelessRFProfile) fromIdentity(ctx context.Context, identity *WirelessRFProfileIdentity) {
...
func (data WirelessRFProfile) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
