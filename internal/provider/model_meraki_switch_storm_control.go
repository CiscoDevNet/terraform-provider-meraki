...
func (data SwitchStormControl) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("broadcastThreshold"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "broadcastThreshold", "null")
		} else {
			body, _ = sjson.Set(body, "broadcastThreshold", value.Int())
		}
	}
	if value := res.Get("multicastThreshold"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "multicastThreshold", "null")
		} else {
			body, _ = sjson.Set(body, "multicastThreshold", value.Int())
		}
	}
	if value := res.Get("unknownUnicastThreshold"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "unknownUnicastThreshold", "null")
		} else {
			body, _ = sjson.Set(body, "unknownUnicastThreshold", value.Int())
		}
	}
	if value := res.Get("treatTheseTrafficTypesAsOneThreshold"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "treatTheseTrafficTypesAsOneThreshold", "null")
		} else {
			var values []string
...
func (data *SwitchStormControl) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data SwitchStormControl) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
