...
// It emits a separate model - DataSourceApplianceInboundCellularFirewallRules - used only by the data source, always including every
...
type DataSourceApplianceInboundCellularFirewallRules struct {
	Id        types.String                                           `tfsdk:"id"`
	NetworkId types.String                                           `tfsdk:"network_id"`
	Rules     []DataSourceApplianceInboundCellularFirewallRulesRules `tfsdk:"rules"`
}

type DataSourceApplianceInboundCellularFirewallRulesRules struct {
...
func (data DataSourceApplianceInboundCellularFirewallRules) getPath() string {
...
func (data *DataSourceApplianceInboundCellularFirewallRules) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceApplianceInboundCellularFirewallRulesRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			if value := res.Get("comment"); value.String() == "Default rule" {
				return true
			}
			parent := &data
			data := DataSourceApplianceInboundCellularFirewallRulesRules{}
