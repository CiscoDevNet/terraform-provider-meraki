...
func (data SwitchSTP) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("stpBridgePriority"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "stpBridgePriority", "null")
		} else {
			body, _ = sjson.Set(body, "stpBridgePriority", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("stpPriority"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "stpPriority", "null")
					} else {
						body, _ = sjson.Set(body, "stpPriority", value.Int())
					}
				}
				...
				*parent, _ = sjson.SetRaw(*parent, "stpBridgePriority.-1", body)
				return true
			})
		}
	}
