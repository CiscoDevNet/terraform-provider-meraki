...
func (data CameraRole) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("appliedOnDevices"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "appliedOnDevices", "null")
		} else {
			body, _ = sjson.Set(body, "appliedOnDevices", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("inNetworksWithId"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "inNetworksWithId", "null")
					} else {
						body, _ = sjson.Set(body, "inNetworksWithId", value.String())
					}
				}
				if value := res.Get("inNetworksWithTag"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "inNetworksWithTag", "null")
					} else {
						body, _ = sjson.Set(body, "inNetworksWithTag", value.String())
					}
				}
				if value := res.Get("permissionScopeId"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "permissionScopeId", "null")
					} else {
						body, _ = sjson.Set(body, "permissionScopeId", value.String())
					}
				}
				if value := res.Get("tag"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "tag", "null")
					} else {
						body, _ = sjson.Set(body, "tag", value.String())
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "appliedOnDevices.-1", body)
				return true
			})
		}
	}
	if value := res.Get("appliedOnNetworks"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "appliedOnNetworks", "null")
		} else {
			body, _ = sjson.Set(body, "appliedOnNetworks", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "appliedOnNetworks.-1", body)
				return true
			})
		}
	}
	if value := res.Get("appliedOrgWide"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "appliedOrgWide", "null")
		} else {
			body, _ = sjson.Set(body, "appliedOrgWide", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "appliedOrgWide.-1", body)
				return true
			})
		}
	}
