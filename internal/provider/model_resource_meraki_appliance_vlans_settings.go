...
func (data ApplianceVLANsSettings) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("vlansEnabled"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "vlansEnabled", "null")
		} else {
			body, _ = sjson.Set(body, "vlansEnabled", value.Bool())
		}
	}
	return body
}
