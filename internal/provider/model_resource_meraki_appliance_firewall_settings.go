...
func (data ApplianceFirewallSettings) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("spoofingProtection.ipSourceGuard.mode"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "spoofingProtection.ipSourceGuard.mode", "null")
		} else {
			body, _ = sjson.Set(body, "spoofingProtection.ipSourceGuard.mode", value.String())
		}
	}
	return body
}
