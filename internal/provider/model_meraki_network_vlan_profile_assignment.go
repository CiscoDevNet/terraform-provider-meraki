...
func (data NetworkVLANProfileAssignment) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("vlanProfile.iname"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "vlanProfile.iname", "null")
		} else {
			body, _ = sjson.Set(body, "vlanProfile.iname", value.String())
		}
	}
	...
	if value := res.Get("stackIds"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "stackIds", "null")
		} else {
			var values []string
			for _, v := range value.Array() {
				values = append(values, v.String())
