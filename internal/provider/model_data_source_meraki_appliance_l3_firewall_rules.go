...
// It emits a separate model - DataSourceApplianceL3FirewallRules - used only by the data source, always including every
...
type DataSourceApplianceL3FirewallRules struct {
	Id                types.String                              `tfsdk:"id"`
	NetworkId         types.String                              `tfsdk:"network_id"`
	SyslogDefaultRule types.Bool                                `tfsdk:"syslog_default_rule"`
	Rules             []DataSourceApplianceL3FirewallRulesRules `tfsdk:"rules"`
}

type DataSourceApplianceL3FirewallRulesRules struct {
...
func (data DataSourceApplianceL3FirewallRules) getPath() string {
...
func (data *DataSourceApplianceL3FirewallRules) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceApplianceL3FirewallRulesRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			if value := res.Get("comment"); value.String() == "Default rule" {
				return true
			}
			parent := &data
			data := DataSourceApplianceL3FirewallRulesRules{}
