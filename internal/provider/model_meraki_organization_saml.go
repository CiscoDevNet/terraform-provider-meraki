...
func (data OrganizationSAML) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("spInitiated.idpId"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "spInitiated.idpId", "null")
		} else {
			body, _ = sjson.Set(body, "spInitiated.idpId", value.String())
		}
	}
	if value := res.Get("spInitiated.subdomain"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "spInitiated.subdomain", "null")
		} else {
			body, _ = sjson.Set(body, "spInitiated.subdomain", value.String())
		}
	}
	return body
}
...
func (data *OrganizationSAML) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
