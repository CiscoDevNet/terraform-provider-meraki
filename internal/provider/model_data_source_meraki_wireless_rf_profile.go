...
// It emits a separate model - DataSourceWirelessRFProfile - used only by the data source, always including every
...
type DataSourceWirelessRFProfile struct {
	...
	FlexRadiosByModel                    []DataSourceWirelessRFProfileFlexRadiosByModel `tfsdk:"flex_radios_by_model"`
...
type DataSourceWirelessRFProfileFlexRadiosByModel struct {
...
func (data DataSourceWirelessRFProfile) getPath() string {
...
func (data *DataSourceWirelessRFProfile) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("flexRadios.byModel"); value.Exists() && value.Value() != nil {
		data.FlexRadiosByModel = make([]DataSourceWirelessRFProfileFlexRadiosByModel, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceWirelessRFProfileFlexRadiosByModel{}
