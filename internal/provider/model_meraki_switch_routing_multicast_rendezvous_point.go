...
func (data SwitchRoutingMulticastRendezvousPoint) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("multicastGroup"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "multicastGroup", "null")
		} else {
			body, _ = sjson.Set(body, "multicastGroup", value.String())
		}
	}
