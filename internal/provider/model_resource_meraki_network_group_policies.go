...
type ResourceNetworkGroupPolicies struct {
	Id             types.String                        `tfsdk:"id"`
	OrganizationId types.String                        `tfsdk:"organization_id"`
	NetworkId      types.String                        `tfsdk:"network_id"`
	Items          []ResourceNetworkGroupPoliciesItems `tfsdk:"items"`
}

type ResourceNetworkGroupPoliciesItems struct {
	Id                                           types.String                                         `tfsdk:"id"`
	...
	BonjourForwardingRules                       []ResourceNetworkGroupPoliciesBonjourForwardingRules `tfsdk:"bonjour_forwarding_rules"`
	...
	L3FirewallRules                              []ResourceNetworkGroupPoliciesL3FirewallRules        `tfsdk:"l3_firewall_rules"`
	L7FirewallRules                              []ResourceNetworkGroupPoliciesL7FirewallRules        `tfsdk:"l7_firewall_rules"`
	TrafficShapingRules                          []ResourceNetworkGroupPoliciesTrafficShapingRules    `tfsdk:"traffic_shaping_rules"`
...
type ResourceNetworkGroupPoliciesBonjourForwardingRules struct {
	Description types.String `tfsdk:"description"`
	VlanId      types.String `tfsdk:"vlan_id"`
	Services    types.Set    `tfsdk:"services"`
}

type ResourceNetworkGroupPoliciesL3FirewallRules struct {
...
type ResourceNetworkGroupPoliciesL7FirewallRules struct {
	Policy types.String `tfsdk:"policy"`
	Type   types.String `tfsdk:"type"`
	Value  types.String `tfsdk:"value"`
}

type ResourceNetworkGroupPoliciesTrafficShapingRules struct {
	...
	Definitions                                      []ResourceNetworkGroupPoliciesTrafficShapingRulesDefinitions `tfsdk:"definitions"`
}

type ResourceNetworkGroupPoliciesTrafficShapingRulesDefinitions struct {
	Type  types.String `tfsdk:"type"`
	Value types.String `tfsdk:"value"`
}
...
func (data ResourceNetworkGroupPolicies) getPath() string {
...
func (data ResourceNetworkGroupPoliciesItems) toBody(ctx context.Context, state ResourceNetworkGroupPoliciesItems) string {
	body := ""
	if !data.Name.IsNull() {
...
func (data *ResourceNetworkGroupPolicies) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]ResourceNetworkGroupPoliciesItems, 0)
	if res.Get("items").Exists() {
		res = meraki.Res{Result: res.Get("items")}
	}
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := ResourceNetworkGroupPoliciesItems{}
	...
	index := 0
	res.ForEach(func(k, res gjson.Result) bool {
		data.Items[index].Id = types.StringValue(res.Get("groupPolicyId").String())
		index++
		return true
	})
...
func (data *ResourceNetworkGroupPolicies) fromBodyPartial(ctx context.Context, res meraki.Res) {
	if res.Get("items").Exists() {
		res = meraki.Res{Result: res.Get("items")}
	}
	...
	for i := range data.Items {
		...
		parentRes.ForEach(
			func(_, v gjson.Result) bool {
				if v.Get("groupPolicyId").String() == (*parent).Items[i].Id.ValueString() {
					res = v
					found = true
					return false
				}
...
func (data *ResourceNetworkGroupPolicies) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data *ResourceNetworkGroupPolicies) fromBodyImport(ctx context.Context, res meraki.Res) {
	if res.Get("items").Exists() {
		res = meraki.Res{Result: res.Get("items")}
	}
	toBeDeleted := make([]int, 0)
...
func (data ResourceNetworkGroupPolicies) addDeleteValues(ctx context.Context) string {
	body := ""
	return body
}
...
func (data *ResourceNetworkGroupPolicies) hasChanges(ctx context.Context, state *ResourceNetworkGroupPolicies, id string) bool {
	...
	stateItem := ResourceNetworkGroupPoliciesItems{}
	...
	if !item.SplashAuthSettings.Equal(stateItem.SplashAuthSettings) {
		hasChanges = true
	}
	if !item.BandwidthSettings.Equal(stateItem.BandwidthSettings) {
		hasChanges = true
	}
	if !item.BandwidthLimitDown.Equal(stateItem.BandwidthLimitDown) {
		hasChanges = true
	}
	if !item.BandwidthLimitUp.Equal(stateItem.BandwidthLimitUp) {
		hasChanges = true
	}
	if !item.BonjourForwardingSettings.Equal(stateItem.BonjourForwardingSettings) {
		hasChanges = true
	}
	if len(item.BonjourForwardingRules) != len(stateItem.BonjourForwardingRules) {
		hasChanges = true
	} else {
		for i := range item.BonjourForwardingRules {
			if !item.BonjourForwardingRules[i].Description.Equal(stateItem.BonjourForwardingRules[i].Description) {
				hasChanges = true
			}
			if !item.BonjourForwardingRules[i].VlanId.Equal(stateItem.BonjourForwardingRules[i].VlanId) {
				hasChanges = true
			}
			if !item.BonjourForwardingRules[i].Services.Equal(stateItem.BonjourForwardingRules[i].Services) {
				hasChanges = true
			}
		}
	}
	if !item.ContentFilteringAllowedUrlPatternsSettings.Equal(stateItem.ContentFilteringAllowedUrlPatternsSettings) {
		hasChanges = true
	}
	if !item.ContentFilteringAllowedUrlPatterns.Equal(stateItem.ContentFilteringAllowedUrlPatterns) {
		hasChanges = true
	}
	if !item.ContentFilteringBlockedUrlCategoriesSettings.Equal(stateItem.ContentFilteringBlockedUrlCategoriesSettings) {
		hasChanges = true
	}
	if !item.ContentFilteringBlockedUrlCategories.Equal(stateItem.ContentFilteringBlockedUrlCategories) {
		hasChanges = true
	}
	if !item.ContentFilteringBlockedUrlPatternsSettings.Equal(stateItem.ContentFilteringBlockedUrlPatternsSettings) {
		hasChanges = true
	}
	if !item.ContentFilteringBlockedUrlPatterns.Equal(stateItem.ContentFilteringBlockedUrlPatterns) {
		hasChanges = true
	}
	if !item.FirewallAndTrafficShapingSettings.Equal(stateItem.FirewallAndTrafficShapingSettings) {
		hasChanges = true
	}
	if len(item.L3FirewallRules) != len(stateItem.L3FirewallRules) {
		hasChanges = true
	} else {
		for i := range item.L3FirewallRules {
			if !item.L3FirewallRules[i].Comment.Equal(stateItem.L3FirewallRules[i].Comment) {
				hasChanges = true
			}
			if !item.L3FirewallRules[i].DestCidr.Equal(stateItem.L3FirewallRules[i].DestCidr) {
				hasChanges = true
			}
			if !item.L3FirewallRules[i].DestPort.Equal(stateItem.L3FirewallRules[i].DestPort) {
				hasChanges = true
			}
			if !item.L3FirewallRules[i].Policy.Equal(stateItem.L3FirewallRules[i].Policy) {
				hasChanges = true
			}
			if !item.L3FirewallRules[i].Protocol.Equal(stateItem.L3FirewallRules[i].Protocol) {
				hasChanges = true
			}
		}
	}
	if len(item.L7FirewallRules) != len(stateItem.L7FirewallRules) {
		hasChanges = true
	} else {
		for i := range item.L7FirewallRules {
			if !item.L7FirewallRules[i].Policy.Equal(stateItem.L7FirewallRules[i].Policy) {
				hasChanges = true
			}
			if !item.L7FirewallRules[i].Type.Equal(stateItem.L7FirewallRules[i].Type) {
				hasChanges = true
			}
			if !item.L7FirewallRules[i].Value.Equal(stateItem.L7FirewallRules[i].Value) {
				hasChanges = true
			}
		}
	}
	if len(item.TrafficShapingRules) != len(stateItem.TrafficShapingRules) {
		hasChanges = true
	} else {
		for i := range item.TrafficShapingRules {
			if !item.TrafficShapingRules[i].DscpTagValue.Equal(stateItem.TrafficShapingRules[i].DscpTagValue) {
				hasChanges = true
			}
			if !item.TrafficShapingRules[i].PcpTagValue.Equal(stateItem.TrafficShapingRules[i].PcpTagValue) {
				hasChanges = true
			}
			if !item.TrafficShapingRules[i].Priority.Equal(stateItem.TrafficShapingRules[i].Priority) {
				hasChanges = true
			}
			if !item.TrafficShapingRules[i].PerClientBandwidthLimitsSettings.Equal(stateItem.TrafficShapingRules[i].PerClientBandwidthLimitsSettings) {
				hasChanges = true
			}
			if !item.TrafficShapingRules[i].PerClientBandwidthLimitsBandwidthLimitsLimitDown.Equal(stateItem.TrafficShapingRules[i].PerClientBandwidthLimitsBandwidthLimitsLimitDown) {
				hasChanges = true
			}
			if !item.TrafficShapingRules[i].PerClientBandwidthLimitsBandwidthLimitsLimitUp.Equal(stateItem.TrafficShapingRules[i].PerClientBandwidthLimitsBandwidthLimitsLimitUp) {
				hasChanges = true
			}
			if len(item.TrafficShapingRules[i].Definitions) != len(stateItem.TrafficShapingRules[i].Definitions) {
				hasChanges = true
			} else {
				for ii := range item.TrafficShapingRules[i].Definitions {
					if !item.TrafficShapingRules[i].Definitions[ii].Type.Equal(stateItem.TrafficShapingRules[i].Definitions[ii].Type) {
						hasChanges = true
					}
					if !item.TrafficShapingRules[i].Definitions[ii].Value.Equal(stateItem.TrafficShapingRules[i].Definitions[ii].Value) {
						hasChanges = true
					}
				}
			}
	...
	if !item.SchedulingEnabled.Equal(stateItem.SchedulingEnabled) {
		hasChanges = true
	}
	if !item.SchedulingFridayActive.Equal(stateItem.SchedulingFridayActive) {
		hasChanges = true
	}
	if !item.SchedulingFridayFrom.Equal(stateItem.SchedulingFridayFrom) {
		hasChanges = true
	}
	if !item.SchedulingFridayTo.Equal(stateItem.SchedulingFridayTo) {
		hasChanges = true
	}
	if !item.SchedulingMondayActive.Equal(stateItem.SchedulingMondayActive) {
		hasChanges = true
	}
	if !item.SchedulingMondayFrom.Equal(stateItem.SchedulingMondayFrom) {
		hasChanges = true
	}
	if !item.SchedulingMondayTo.Equal(stateItem.SchedulingMondayTo) {
		hasChanges = true
	}
	if !item.SchedulingSaturdayActive.Equal(stateItem.SchedulingSaturdayActive) {
		hasChanges = true
	}
	if !item.SchedulingSaturdayFrom.Equal(stateItem.SchedulingSaturdayFrom) {
		hasChanges = true
	}
	if !item.SchedulingSaturdayTo.Equal(stateItem.SchedulingSaturdayTo) {
		hasChanges = true
	}
	if !item.SchedulingSundayActive.Equal(stateItem.SchedulingSundayActive) {
		hasChanges = true
	}
	if !item.SchedulingSundayFrom.Equal(stateItem.SchedulingSundayFrom) {
		hasChanges = true
	}
	if !item.SchedulingSundayTo.Equal(stateItem.SchedulingSundayTo) {
		hasChanges = true
	}
	if !item.SchedulingThursdayActive.Equal(stateItem.SchedulingThursdayActive) {
		hasChanges = true
	}
	if !item.SchedulingThursdayFrom.Equal(stateItem.SchedulingThursdayFrom) {
		hasChanges = true
	}
	if !item.SchedulingThursdayTo.Equal(stateItem.SchedulingThursdayTo) {
		hasChanges = true
	}
	if !item.SchedulingTuesdayActive.Equal(stateItem.SchedulingTuesdayActive) {
		hasChanges = true
	}
	if !item.SchedulingTuesdayFrom.Equal(stateItem.SchedulingTuesdayFrom) {
		hasChanges = true
	}
	if !item.SchedulingTuesdayTo.Equal(stateItem.SchedulingTuesdayTo) {
		hasChanges = true
	}
	if !item.SchedulingWednesdayActive.Equal(stateItem.SchedulingWednesdayActive) {
		hasChanges = true
	}
	if !item.SchedulingWednesdayFrom.Equal(stateItem.SchedulingWednesdayFrom) {
		hasChanges = true
	}
	if !item.SchedulingWednesdayTo.Equal(stateItem.SchedulingWednesdayTo) {
		hasChanges = true
	}
	if !item.VlanTaggingSettings.Equal(stateItem.VlanTaggingSettings) {
		hasChanges = true
	}
	if !item.VlanTaggingVlanId.Equal(stateItem.VlanTaggingVlanId) {
		hasChanges = true
	}
	return hasChanges
}
