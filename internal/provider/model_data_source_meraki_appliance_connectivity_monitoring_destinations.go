...
// It emits a separate model - DataSourceApplianceConnectivityMonitoringDestinations - used only by the data source, always including every
...
type DataSourceApplianceConnectivityMonitoringDestinations struct {
	Id           types.String                                                        `tfsdk:"id"`
	NetworkId    types.String                                                        `tfsdk:"network_id"`
	Destinations []DataSourceApplianceConnectivityMonitoringDestinationsDestinations `tfsdk:"destinations"`
}

type DataSourceApplianceConnectivityMonitoringDestinationsDestinations struct {
...
func (data DataSourceApplianceConnectivityMonitoringDestinations) getPath() string {
...
func (data *DataSourceApplianceConnectivityMonitoringDestinations) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("destinations"); value.Exists() && value.Value() != nil {
		data.Destinations = make([]DataSourceApplianceConnectivityMonitoringDestinationsDestinations, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceApplianceConnectivityMonitoringDestinationsDestinations{}
