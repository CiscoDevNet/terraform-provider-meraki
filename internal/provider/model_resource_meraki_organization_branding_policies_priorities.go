...
func (data OrganizationBrandingPoliciesPriorities) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("brandingPolicyIds"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "brandingPolicyIds", "null")
		} else {
			var values []string
...
func (data *OrganizationBrandingPoliciesPriorities) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
