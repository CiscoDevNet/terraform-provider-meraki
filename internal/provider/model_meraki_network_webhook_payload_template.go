...
type NetworkWebhookPayloadTemplateIdentity struct {
	NetworkId types.String `tfsdk:"network_id"`
	Id        types.String `tfsdk:"id"`
}
...
func (data NetworkWebhookPayloadTemplate) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("body"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "body", "null")
		} else {
			body, _ = sjson.Set(body, "body", value.String())
		}
	}
	if value := res.Get("bodyFile"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "bodyFile", "null")
		} else {
			body, _ = sjson.Set(body, "bodyFile", value.String())
		}
	}
	if value := res.Get("headersFile"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "headersFile", "null")
		} else {
			body, _ = sjson.Set(body, "headersFile", value.String())
		}
	}
	...
	if value := res.Get("headers"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "headers", "null")
		} else {
			body, _ = sjson.Set(body, "headers", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("template"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "template", "null")
					} else {
						body, _ = sjson.Set(body, "template", value.String())
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "headers.-1", body)
				return true
			})
		}
	}
...
func (data *NetworkWebhookPayloadTemplate) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data *NetworkWebhookPayloadTemplateIdentity) toIdentity(ctx context.Context, plan *NetworkWebhookPayloadTemplate) {
...
func (data *NetworkWebhookPayloadTemplate) fromIdentity(ctx context.Context, identity *NetworkWebhookPayloadTemplateIdentity) {
...
func (data NetworkWebhookPayloadTemplate) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
