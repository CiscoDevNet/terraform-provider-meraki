...
// It emits a separate model - DataSourceNetworkVLANProfile - used only by the data source, always including every
...
type DataSourceNetworkVLANProfile struct {
	Id         types.String                             `tfsdk:"id"`
	NetworkId  types.String                             `tfsdk:"network_id"`
	Iname      types.String                             `tfsdk:"iname"`
	Name       types.String                             `tfsdk:"name"`
	VlanGroups []DataSourceNetworkVLANProfileVlanGroups `tfsdk:"vlan_groups"`
	VlanNames  []DataSourceNetworkVLANProfileVlanNames  `tfsdk:"vlan_names"`
}

type DataSourceNetworkVLANProfileVlanGroups struct {
	Name    types.String `tfsdk:"name"`
	VlanIds types.String `tfsdk:"vlan_ids"`
}

type DataSourceNetworkVLANProfileVlanNames struct {
	Name                  types.String `tfsdk:"name"`
...
func (data DataSourceNetworkVLANProfile) getPath() string {
...
func (data *DataSourceNetworkVLANProfile) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("vlanGroups"); value.Exists() && value.Value() != nil {
		data.VlanGroups = make([]DataSourceNetworkVLANProfileVlanGroups, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceNetworkVLANProfileVlanGroups{}
			if value := res.Get("name"); value.Exists() && value.Value() != nil {
				data.Name = types.StringValue(value.String())
			} else {
				data.Name = types.StringNull()
	...
	if value := res.Get("vlanNames"); value.Exists() && value.Value() != nil {
		data.VlanNames = make([]DataSourceNetworkVLANProfileVlanNames, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			if value := res.Get("name"); value.String() == "default" {
				return true
			}
			parent := &data
			data := DataSourceNetworkVLANProfileVlanNames{}
			if value := res.Get("name"); value.Exists() && value.Value() != nil {
				data.Name = types.StringValue(value.String())
			} else {
				data.Name = types.StringNull()
