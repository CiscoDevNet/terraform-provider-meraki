...
func (data SwitchAccessControlLists) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
		...
		if value.Value() == nil {
			...
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("dstCidr"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "dstCidr", "null")
					} else {
						body, _ = sjson.Set(body, "dstCidr", value.String())
					}
				}
				...
				if value := res.Get("vlan"); value.Exists() {
					if value.Value() == nil {
						...
						body, _ = sjson.Set(body, "vlan", value.String())
					}
				}
