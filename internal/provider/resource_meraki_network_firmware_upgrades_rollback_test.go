...
func TestAccMerakiNetworkFirmwareUpgradesRollback(t *testing.T) {
	...
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccMerakiNetworkFirmwareUpgradesRollbackPrerequisitesConfig + testAccMerakiNetworkFirmwareUpgradesRollbackConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccMerakiNetworkFirmwareUpgradesRollbackPrerequisitesConfig + testAccMerakiNetworkFirmwareUpgradesRollbackConfig_all(),
...
func merakiNetworkFirmwareUpgradesRollbackImportStateIdFunc(resourceName string) resource.ImportStateIdFunc {
...
const testAccMerakiNetworkFirmwareUpgradesRollbackPrerequisitesConfig = `
variable "test_org" {}
...
func testAccMerakiNetworkFirmwareUpgradesRollbackConfig_minimum() string {
	config := `resource "meraki_network_firmware_upgrades_rollback" "test" {` + "\n"
	config += `  network_id = meraki_network.test.id` + "\n"
	config += `  reasons = [{` + "\n"
	config += `    category = "performance"` + "\n"
	config += `    comment = "Network was slower with the upgrade"` + "\n"
	config += `  }]` + "\n"
	config += `}` + "\n"
	return config
}
...
func testAccMerakiNetworkFirmwareUpgradesRollbackConfig_all() string {
	config := `resource "meraki_network_firmware_upgrades_rollback" "test" {` + "\n"
	config += `  network_id = meraki_network.test.id` + "\n"
