...
type NetworkDevices struct {
	...
	Items     []NetworkDevicesItems `tfsdk:"items"`
}

type NetworkDevicesItems struct {
	...
	Details             []NetworkDevicesDetails `tfsdk:"details"`
	Tags                types.List              `tfsdk:"tags"`
}

type NetworkDevicesDetails struct {
	Name  types.String `tfsdk:"name"`
...
func (data *NetworkDevices) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]NetworkDevicesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := NetworkDevicesItems{}
		...
		if value := res.Get("details"); value.Exists() && value.Value() != nil {
			data.Details = make([]NetworkDevicesDetails, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := NetworkDevicesDetails{}
				if value := res.Get("name"); value.Exists() && value.Value() != nil {
					data.Name = types.StringValue(value.String())
				} else {
					data.Name = types.StringNull()
