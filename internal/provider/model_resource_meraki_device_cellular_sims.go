...
func (data DeviceCellularSIMs) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("simFailover.enabled"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "simFailover.enabled", "null")
		} else {
			body, _ = sjson.Set(body, "simFailover.enabled", value.Bool())
		}
	}
	if value := res.Get("simFailover.timeout"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "simFailover.timeout", "null")
		} else {
			body, _ = sjson.Set(body, "simFailover.timeout", value.Int())
		}
	}
	if value := res.Get("simOrdering"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "simOrdering", "null")
		} else {
			var values []string
	...
	if value := res.Get("sims"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "sims", "null")
		} else {
			body, _ = sjson.Set(body, "sims", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("isPrimary"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "isPrimary", "null")
					} else {
						body, _ = sjson.Set(body, "isPrimary", value.Bool())
					}
				}
				if value := res.Get("simOrder"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "simOrder", "null")
					} else {
						body, _ = sjson.Set(body, "simOrder", value.Int())
					}
				}
				if value := res.Get("slot"); value.Exists() {
					if value.Value() == nil {
						...
						body, _ = sjson.Set(body, "slot", value.String())
					}
				}
				if value := res.Get("apns"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "apns", "null")
					} else {
						body, _ = sjson.Set(body, "apns", []interface{}{})
						parent := &body
						value.ForEach(func(k, res gjson.Result) bool {
							body := ""
							...
							if value := res.Get("authentication.type"); value.Exists() {
								if value.Value() == nil {
									body, _ = sjson.SetRaw(body, "authentication.type", "null")
								} else {
									body, _ = sjson.Set(body, "authentication.type", value.String())
								}
							}
							...
							if value := res.Get("allowedIpTypes"); value.Exists() {
								if value.Value() == nil {
									body, _ = sjson.SetRaw(body, "allowedIpTypes", "null")
								} else {
									var values []string
									for _, v := range value.Array() {
										values = append(values, v.String())
									}
									body, _ = sjson.Set(body, "allowedIpTypes", values)
								}
							}
							*parent, _ = sjson.SetRaw(*parent, "apns.-1", body)
							return true
						})
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "sims.-1", body)
				return true
			})
		}
	}
...
func (data *DeviceCellularSIMs) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data DeviceCellularSIMs) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
