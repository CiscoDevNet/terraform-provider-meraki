...
func (data ApplianceVPNSiteToSiteIPsecPeersSLAs) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("items"); value.Exists() {
		if value.Value() == nil {
			...
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("uri"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "uri", "null")
					} else {
						body, _ = sjson.Set(body, "uri", value.String())
					}
				}
