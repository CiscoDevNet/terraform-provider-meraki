...
// It emits a separate model - DataSourceOrganizationAuthRADIUSServer - used only by the data source, always including every
...
type DataSourceOrganizationAuthRADIUSServer struct {
	...
	Modes           []DataSourceOrganizationAuthRADIUSServerModes `tfsdk:"modes"`
}

type DataSourceOrganizationAuthRADIUSServerModes struct {
...
func (data DataSourceOrganizationAuthRADIUSServer) getPath() string {
...
func (data *DataSourceOrganizationAuthRADIUSServer) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("modes"); value.Exists() && value.Value() != nil {
		data.Modes = make([]DataSourceOrganizationAuthRADIUSServerModes, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceOrganizationAuthRADIUSServerModes{}
