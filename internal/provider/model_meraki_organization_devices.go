...
type OrganizationDevices struct {
	...
	Items          []OrganizationDevicesItems `tfsdk:"items"`
}

type OrganizationDevicesItems struct {
	...
	Details     []OrganizationDevicesDetails `tfsdk:"details"`
	Tags        types.List                   `tfsdk:"tags"`
}

type OrganizationDevicesDetails struct {
	Name  types.String `tfsdk:"name"`
...
func (data OrganizationDevices) getPath() string {
...
func (data *OrganizationDevices) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]OrganizationDevicesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := OrganizationDevicesItems{}
		...
		if value := res.Get("details"); value.Exists() && value.Value() != nil {
			data.Details = make([]OrganizationDevicesDetails, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := OrganizationDevicesDetails{}
				if value := res.Get("name"); value.Exists() && value.Value() != nil {
					data.Name = types.StringValue(value.String())
				} else {
					data.Name = types.StringNull()
