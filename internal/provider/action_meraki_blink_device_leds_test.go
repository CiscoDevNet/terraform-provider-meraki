...
func TestAccMerakiBlinkDeviceLeds(t *testing.T) {
	if os.Getenv("BLINK_DEVICE_LEDS") == "" {
		t.Skip("skipping test, set environment variable BLINK_DEVICE_LEDS")
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMerakiBlinkDeviceLedsPrerequisitesConfig + testAccMerakiBlinkDeviceLedsConfig(),
			},
		},
	})
}
...
const testAccMerakiBlinkDeviceLedsPrerequisitesConfig = `
variable "test_org" {}
...
// testAccMerakiBlinkDeviceLedsConfig only sets the id/reference/mandatory top-level
...
func testAccMerakiBlinkDeviceLedsConfig() string {
	config := `action "meraki_blink_device_leds" "test" {` + "\n"
	config += `  config {` + "\n"
