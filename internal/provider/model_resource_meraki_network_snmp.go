...
func (data NetworkSNMP) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("access"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "access", "null")
		} else {
			body, _ = sjson.Set(body, "access", value.String())
		}
	}
	if value := res.Get("communityString"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "communityString", "null")
		} else {
			body, _ = sjson.Set(body, "communityString", value.String())
		}
	}
	if value := res.Get("users"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "users", "null")
		} else {
			body, _ = sjson.Set(body, "users", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("passphrase"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "passphrase", "null")
					} else {
						body, _ = sjson.Set(body, "passphrase", value.String())
					}
				}
				if value := res.Get("username"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "username", "null")
					} else {
						body, _ = sjson.Set(body, "username", value.String())
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "users.-1", body)
				return true
			})
		}
	}
...
func (data *NetworkSNMPIdentity) toIdentity(ctx context.Context, plan *NetworkSNMP) {
...
func (data *NetworkSNMP) fromIdentity(ctx context.Context, identity *NetworkSNMPIdentity) {
