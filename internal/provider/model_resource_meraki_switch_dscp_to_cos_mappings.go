...
func (data SwitchDSCPToCoSMappings) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("mappings"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "mappings", "null")
		} else {
			body, _ = sjson.Set(body, "mappings", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("cos"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "cos", "null")
					} else {
						body, _ = sjson.Set(body, "cos", value.Int())
					}
				}
				...
				if value := res.Get("title"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "title", "null")
					} else {
						body, _ = sjson.Set(body, "title", value.String())
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "mappings.-1", body)
				return true
			})
		}
	}
