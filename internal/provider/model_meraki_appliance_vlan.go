...
type ApplianceVLAN struct {
	...
	FixedIpAssignments     map[string]ApplianceVLANFixedIpAssignments `tfsdk:"fixed_ip_assignments"`
	...
	ReservedIpRanges       []ApplianceVLANReservedIpRanges            `tfsdk:"reserved_ip_ranges"`
}

type ApplianceVLANFixedIpAssignments struct {
...
type ApplianceVLANReservedIpRanges struct {
	Comment types.String `tfsdk:"comment"`
	End     types.String `tfsdk:"end"`
	Start   types.String `tfsdk:"start"`
}

type ApplianceVLANIdentity struct {
	NetworkId types.String `tfsdk:"network_id"`
	Id        types.String `tfsdk:"id"`
}
...
func (data ApplianceVLAN) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("cidr"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "cidr", "null")
		} else {
			body, _ = sjson.Set(body, "cidr", value.String())
		}
	}
	...
	if value := res.Get("mask"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "mask", "null")
		} else {
			body, _ = sjson.Set(body, "mask", value.Int())
		}
	}
	...
	if value := res.Get("templateVlanType"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "templateVlanType", "null")
		} else {
			body, _ = sjson.Set(body, "templateVlanType", value.String())
		}
	}
...
func (data *ApplianceVLAN) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("fixedIpAssignments"); value.Exists() && value.Value() != nil {
		data.FixedIpAssignments = make(map[string]ApplianceVLANFixedIpAssignments)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := ApplianceVLANFixedIpAssignments{}
	...
	if value := res.Get("reservedIpRanges"); value.Exists() && value.Value() != nil {
		data.ReservedIpRanges = make([]ApplianceVLANReservedIpRanges, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := ApplianceVLANReservedIpRanges{}
...
func (data *ApplianceVLANIdentity) toIdentity(ctx context.Context, plan *ApplianceVLAN) {
...
func (data *ApplianceVLAN) fromIdentity(ctx context.Context, identity *ApplianceVLANIdentity) {
