...
// It emits a separate model - DataSourceApplianceVPNFirewallRules - used only by the data source, always including every
...
type DataSourceApplianceVPNFirewallRules struct {
	Id                types.String                               `tfsdk:"id"`
	OrganizationId    types.String                               `tfsdk:"organization_id"`
	SyslogDefaultRule types.Bool                                 `tfsdk:"syslog_default_rule"`
	Rules             []DataSourceApplianceVPNFirewallRulesRules `tfsdk:"rules"`
}

type DataSourceApplianceVPNFirewallRulesRules struct {
...
func (data DataSourceApplianceVPNFirewallRules) getPath() string {
...
func (data *DataSourceApplianceVPNFirewallRules) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceApplianceVPNFirewallRulesRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			if value := res.Get("comment"); value.String() == "Default rule" {
				return true
			}
			parent := &data
			data := DataSourceApplianceVPNFirewallRulesRules{}
