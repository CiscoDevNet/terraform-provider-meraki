...
func (data ApplianceContentFiltering) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("allowedUrlPatterns"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "allowedUrlPatterns", "null")
		} else {
			var values []string
	...
	if value := res.Get("blockedUrlPatterns"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "blockedUrlPatterns", "null")
		} else {
			var values []string
			for _, v := range value.Array() {
				values = append(values, v.String())
