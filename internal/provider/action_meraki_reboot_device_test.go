...
func TestAccMerakiRebootDevice(t *testing.T) {
	if os.Getenv("REBOOT_DEVICE") == "" {
		t.Skip("skipping test, set environment variable REBOOT_DEVICE")
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccMerakiRebootDevicePrerequisitesConfig + testAccMerakiRebootDeviceConfig(),
			},
		},
	})
}
...
const testAccMerakiRebootDevicePrerequisitesConfig = `
variable "test_org" {}
...
// testAccMerakiRebootDeviceConfig only sets the id/reference/mandatory top-level
...
func testAccMerakiRebootDeviceConfig() string {
	config := `action "meraki_reboot_device" "test" {` + "\n"
	config += `  config {` + "\n"
