...
type SMAdminRoleIdentity struct {
	OrganizationId types.String `tfsdk:"organization_id"`
	Id             types.String `tfsdk:"id"`
}
...
func (data SMAdminRole) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("scope"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "scope", "null")
		} else {
			body, _ = sjson.Set(body, "scope", value.String())
		}
	}
...
func (data *SMAdminRoleIdentity) toIdentity(ctx context.Context, plan *SMAdminRole) {
...
func (data *SMAdminRole) fromIdentity(ctx context.Context, identity *SMAdminRoleIdentity) {
