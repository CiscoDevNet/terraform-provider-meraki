...
func (data SwitchQoSRuleOrder) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("ruleIds"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ruleIds", "null")
		} else {
			var values []string
...
func (data *SwitchQoSRuleOrder) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data SwitchQoSRuleOrder) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
