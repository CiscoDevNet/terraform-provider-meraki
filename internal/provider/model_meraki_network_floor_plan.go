...
func (data NetworkFloorPlan) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("floorNumber"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "floorNumber", "null")
		} else {
			body, _ = sjson.Set(body, "floorNumber", value.Float())
		}
	}
...
func (data NetworkFloorPlan) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
