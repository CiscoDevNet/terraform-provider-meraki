...
func (data CameraVideoSettings) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("externalRtspEnabled"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "externalRtspEnabled", "null")
		} else {
			body, _ = sjson.Set(body, "externalRtspEnabled", value.Bool())
		}
	}
	return body
}
