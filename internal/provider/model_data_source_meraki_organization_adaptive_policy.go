...
// It emits a separate model - DataSourceOrganizationAdaptivePolicy - used only by the data source, always including every
...
type DataSourceOrganizationAdaptivePolicy struct {
	...
	Acls                 []DataSourceOrganizationAdaptivePolicyAcls `tfsdk:"acls"`
}
...
func (data DataSourceOrganizationAdaptivePolicy) getPath() string {
...
func (data *DataSourceOrganizationAdaptivePolicy) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("acls"); value.Exists() && value.Value() != nil {
		data.Acls = make([]DataSourceOrganizationAdaptivePolicyAcls, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceOrganizationAdaptivePolicyAcls{}
