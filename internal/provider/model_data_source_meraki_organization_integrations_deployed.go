...
// It emits a separate model - DataSourceOrganizationIntegrationsDeployed - used only by the data source, always including every
...
func (data DataSourceOrganizationIntegrationsDeployed) getPath() string {
...
func (data *DataSourceOrganizationIntegrationsDeployed) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("items"); value.Exists() && value.Value() != nil {
		...
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceOrganizationIntegrationsDeployedItems{}
