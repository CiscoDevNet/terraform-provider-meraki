...
// It emits a separate model - DataSourceWirelessSSIDBonjourForwarding - used only by the data source, always including every
...
type DataSourceWirelessSSIDBonjourForwarding struct {
	Id               types.String                                   `tfsdk:"id"`
	NetworkId        types.String                                   `tfsdk:"network_id"`
	Number           types.String                                   `tfsdk:"number"`
	Enabled          types.Bool                                     `tfsdk:"enabled"`
	ExceptionEnabled types.Bool                                     `tfsdk:"exception_enabled"`
	Rules            []DataSourceWirelessSSIDBonjourForwardingRules `tfsdk:"rules"`
}

type DataSourceWirelessSSIDBonjourForwardingRules struct {
...
func (data DataSourceWirelessSSIDBonjourForwarding) getPath() string {
...
func (data *DataSourceWirelessSSIDBonjourForwarding) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceWirelessSSIDBonjourForwardingRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceWirelessSSIDBonjourForwardingRules{}
