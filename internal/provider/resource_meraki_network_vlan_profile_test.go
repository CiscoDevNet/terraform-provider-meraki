...
func TestAccMerakiNetworkVLANProfile(t *testing.T) {
	...
	steps = append(steps, resource.TestStep{
		ResourceName:            "meraki_network_vlan_profile.test",
		ImportState:             true,
...
func merakiNetworkVLANProfileImportStateIdFunc(resourceName string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		primary := s.RootModule().Resources[resourceName].Primary
		NetworkId := primary.Attributes["network_id"]
		Iname := primary.Attributes["iname"]

		return fmt.Sprintf("%s,%s", NetworkId, Iname), nil
	}
}
