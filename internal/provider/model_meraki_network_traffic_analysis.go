...
func (data NetworkTrafficAnalysis) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("customPieChartItems"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "customPieChartItems", "null")
		} else {
			body, _ = sjson.Set(body, "customPieChartItems", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "customPieChartItems.-1", body)
				return true
			})
		}
	}
...
func (data NetworkTrafficAnalysis) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
