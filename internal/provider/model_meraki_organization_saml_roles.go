...
type OrganizationSAMLRoles struct {
	OrganizationId types.String                 `tfsdk:"organization_id"`
	Items          []OrganizationSAMLRolesItems `tfsdk:"items"`
}

type OrganizationSAMLRolesItems struct {
	Id        types.String                    `tfsdk:"id"`
	OrgAccess types.String                    `tfsdk:"org_access"`
	Role      types.String                    `tfsdk:"role"`
	Networks  []OrganizationSAMLRolesNetworks `tfsdk:"networks"`
	Tags      []OrganizationSAMLRolesTags     `tfsdk:"tags"`
}

type OrganizationSAMLRolesNetworks struct {
	Access types.String `tfsdk:"access"`
	Id     types.String `tfsdk:"id"`
}

type OrganizationSAMLRolesTags struct {
	Access types.String `tfsdk:"access"`
...
func (data OrganizationSAMLRoles) getPath() string {
...
func (data *OrganizationSAMLRoles) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]OrganizationSAMLRolesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := OrganizationSAMLRolesItems{}
		...
		if value := res.Get("networks"); value.Exists() && value.Value() != nil {
			data.Networks = make([]OrganizationSAMLRolesNetworks, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := OrganizationSAMLRolesNetworks{}
		...
		if value := res.Get("tags"); value.Exists() && value.Value() != nil {
			data.Tags = make([]OrganizationSAMLRolesTags, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := OrganizationSAMLRolesTags{}
				if value := res.Get("access"); value.Exists() && value.Value() != nil {
					data.Access = types.StringValue(value.String())
				} else {
					data.Access = types.StringNull()
