...
func (data OrganizationAdaptivePolicyGroup) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("sgt"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "sgt", "null")
		} else {
			body, _ = sjson.Set(body, "sgt", value.Int())
		}
	}
	if value := res.Get("policyObjects"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "policyObjects", "null")
		} else {
			body, _ = sjson.Set(body, "policyObjects", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "policyObjects.-1", body)
				return true
			})
		}
	}
...
func (data *OrganizationAdaptivePolicyGroup) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data OrganizationAdaptivePolicyGroup) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
