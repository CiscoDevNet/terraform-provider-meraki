...
type DataSourceWirelessRFProfiles struct {
	NetworkId types.String                        `tfsdk:"network_id"`
	Items     []DataSourceWirelessRFProfilesItems `tfsdk:"items"`
}

type DataSourceWirelessRFProfilesItems struct {
	...
	FlexRadiosByModel                    []DataSourceWirelessRFProfilesFlexRadiosByModel `tfsdk:"flex_radios_by_model"`
...
func (data DataSourceWirelessRFProfiles) getPath() string {
...
func (data *DataSourceWirelessRFProfiles) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]DataSourceWirelessRFProfilesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := DataSourceWirelessRFProfilesItems{}
		...
		if value := res.Get("flexRadios.byModel"); value.Exists() && value.Value() != nil {
			data.FlexRadiosByModel = make([]DataSourceWirelessRFProfilesFlexRadiosByModel, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := DataSourceWirelessRFProfilesFlexRadiosByModel{}
