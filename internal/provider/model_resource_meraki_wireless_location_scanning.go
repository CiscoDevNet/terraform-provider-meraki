...
func (data WirelessLocationScanning) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("api.enabled"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "api.enabled", "null")
		} else {
			body, _ = sjson.Set(body, "api.enabled", value.Bool())
		}
	}
	return body
}
