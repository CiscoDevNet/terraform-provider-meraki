...
type ResourceOrganizationPolicyObjectsIdentity struct {
	OrganizationId types.String `tfsdk:"organization_id"`
...
func (data *ResourceOrganizationPolicyObjects) fromBodyImport(ctx context.Context, res meraki.Res) {
	...
	for i := range data.Items {
		...
		if value := res.Get("groupIds"); value.Exists() && value.Value() != nil && len(value.Array()) > 0 {
			data.GroupIds = helpers.GetStringSet(value.Array())
		} else {
			data.GroupIds = types.SetNull(types.StringType)
		}
...
func (data *ResourceOrganizationPolicyObjects) fromIdentity(ctx context.Context, identity *ResourceOrganizationPolicyObjectsIdentity) {
