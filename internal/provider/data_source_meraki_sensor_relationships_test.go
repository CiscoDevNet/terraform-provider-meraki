...
func TestAccDataSourceMerakiSensorRelationships(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("data.meraki_sensor_relationships.test", "livestream_related_devices.0.serial", ""))
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceMerakiSensorRelationshipsPrerequisitesConfig + testAccDataSourceMerakiSensorRelationshipsConfig(),
...
const testAccDataSourceMerakiSensorRelationshipsPrerequisitesConfig = `
variable "test_org" {}
...
func testAccDataSourceMerakiSensorRelationshipsConfig() string {
	...
	config += `
		data "meraki_sensor_relationships" "test" {
			serial = tolist(meraki_network_device_claim.test.serials)[0]
			depends_on = [meraki_sensor_relationships.test]
		}
	`
	return config
}
