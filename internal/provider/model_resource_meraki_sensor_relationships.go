...
type SensorRelationshipsIdentity struct {
	Serial types.String `tfsdk:"serial"`
}
...
func (data SensorRelationships) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("livestream.relatedDevices"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "livestream.relatedDevices", "null")
		} else {
			body, _ = sjson.Set(body, "livestream.relatedDevices", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "livestream.relatedDevices.-1", body)
				return true
			})
		}
	}
...
func (data *SensorRelationshipsIdentity) toIdentity(ctx context.Context, plan *SensorRelationships) {
...
func (data *SensorRelationships) fromIdentity(ctx context.Context, identity *SensorRelationshipsIdentity) {
