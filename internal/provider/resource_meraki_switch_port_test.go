...
func TestAccMerakiSwitchPort(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("meraki_switch_port.test", "poe_enabled", "false"))
	...
	steps = append(steps, resource.TestStep{
		ResourceName:            "meraki_switch_port.test",
		ImportState:             true,
		ImportStateVerify:       true,
		ImportStateIdFunc:       merakiSwitchPortImportStateIdFunc("meraki_switch_port.test"),
		ImportStateVerifyIgnore: []string{"profile_id"},
		Check:                   resource.ComposeTestCheckFunc(checks...),
	})
...
func merakiSwitchPortImportStateIdFunc(resourceName string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		primary := s.RootModule().Resources[resourceName].Primary
		Serial := primary.Attributes["serial"]
		PortId := primary.Attributes["port_id"]

		return fmt.Sprintf("%s,%s", Serial, PortId), nil
	}
}
