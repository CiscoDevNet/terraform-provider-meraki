...
// It emits a separate model - DataSourceSensorAlertsProfile - used only by the data source, always including every
...
type DataSourceSensorAlertsProfile struct {
	...
	Conditions              []DataSourceSensorAlertsProfileConditions `tfsdk:"conditions"`
	Serials                 types.List                                `tfsdk:"serials"`
}

type DataSourceSensorAlertsProfileConditions struct {
...
func (data DataSourceSensorAlertsProfile) getPath() string {
...
func (data *DataSourceSensorAlertsProfile) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("conditions"); value.Exists() && value.Value() != nil {
		data.Conditions = make([]DataSourceSensorAlertsProfileConditions, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceSensorAlertsProfileConditions{}
