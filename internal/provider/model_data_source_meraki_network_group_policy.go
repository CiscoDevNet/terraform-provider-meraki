...
// It emits a separate model - DataSourceNetworkGroupPolicy - used only by the data source, always including every
...
type DataSourceNetworkGroupPolicy struct {
	...
	BonjourForwardingRules                       []DataSourceNetworkGroupPolicyBonjourForwardingRules `tfsdk:"bonjour_forwarding_rules"`
	...
	L3FirewallRules                              []DataSourceNetworkGroupPolicyL3FirewallRules        `tfsdk:"l3_firewall_rules"`
	L7FirewallRules                              []DataSourceNetworkGroupPolicyL7FirewallRules        `tfsdk:"l7_firewall_rules"`
	TrafficShapingRules                          []DataSourceNetworkGroupPolicyTrafficShapingRules    `tfsdk:"traffic_shaping_rules"`
...
type DataSourceNetworkGroupPolicyBonjourForwardingRules struct {
	Description types.String `tfsdk:"description"`
	VlanId      types.String `tfsdk:"vlan_id"`
	Services    types.Set    `tfsdk:"services"`
}

type DataSourceNetworkGroupPolicyL3FirewallRules struct {
...
type DataSourceNetworkGroupPolicyL7FirewallRules struct {
	Policy types.String `tfsdk:"policy"`
	Type   types.String `tfsdk:"type"`
	Value  types.String `tfsdk:"value"`
}

type DataSourceNetworkGroupPolicyTrafficShapingRules struct {
	...
	Definitions                                      []DataSourceNetworkGroupPolicyTrafficShapingRulesDefinitions `tfsdk:"definitions"`
}

type DataSourceNetworkGroupPolicyTrafficShapingRulesDefinitions struct {
	Type  types.String `tfsdk:"type"`
	Value types.String `tfsdk:"value"`
}
...
func (data DataSourceNetworkGroupPolicy) getPath() string {
...
// `type` discriminates whether `value` is read from the raw `value` path or `value.id` for the L7 firewall rules
// and traffic shaping rule definitions.

func (data *DataSourceNetworkGroupPolicy) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("bonjourForwarding.rules"); value.Exists() && value.Value() != nil {
		data.BonjourForwardingRules = make([]DataSourceNetworkGroupPolicyBonjourForwardingRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceNetworkGroupPolicyBonjourForwardingRules{}
	...
	if value := res.Get("firewallAndTrafficShaping.l3FirewallRules"); value.Exists() && value.Value() != nil {
		data.L3FirewallRules = make([]DataSourceNetworkGroupPolicyL3FirewallRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceNetworkGroupPolicyL3FirewallRules{}
	...
	if value := res.Get("firewallAndTrafficShaping.l7FirewallRules"); value.Exists() && value.Value() != nil {
		data.L7FirewallRules = make([]DataSourceNetworkGroupPolicyL7FirewallRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceNetworkGroupPolicyL7FirewallRules{}
			if value := res.Get("policy"); value.Exists() && value.Value() != nil {
				data.Policy = types.StringValue(value.String())
			} else {
				data.Policy = types.StringNull()
	...
	if value := res.Get("firewallAndTrafficShaping.trafficShapingRules"); value.Exists() && value.Value() != nil {
		data.TrafficShapingRules = make([]DataSourceNetworkGroupPolicyTrafficShapingRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceNetworkGroupPolicyTrafficShapingRules{}
			...
			if value := res.Get("definitions"); value.Exists() && value.Value() != nil {
				data.Definitions = make([]DataSourceNetworkGroupPolicyTrafficShapingRulesDefinitions, 0)
				value.ForEach(func(k, res gjson.Result) bool {
					parent := &data
					data := DataSourceNetworkGroupPolicyTrafficShapingRulesDefinitions{}
					if value := res.Get("type"); value.Exists() && value.Value() != nil {
						data.Type = types.StringValue(value.String())
					} else {
						data.Type = types.StringNull()
