...
// It emits a separate model - DataSourceNetworkWebhookPayloadTemplate - used only by the data source, always including every
...
type DataSourceNetworkWebhookPayloadTemplate struct {
	...
	Headers     []DataSourceNetworkWebhookPayloadTemplateHeaders `tfsdk:"headers"`
}

type DataSourceNetworkWebhookPayloadTemplateHeaders struct {
	Name     types.String `tfsdk:"name"`
...
func (data DataSourceNetworkWebhookPayloadTemplate) getPath() string {
...
func (data *DataSourceNetworkWebhookPayloadTemplate) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("headers"); value.Exists() && value.Value() != nil {
		data.Headers = make([]DataSourceNetworkWebhookPayloadTemplateHeaders, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceNetworkWebhookPayloadTemplateHeaders{}
			if value := res.Get("name"); value.Exists() && value.Value() != nil {
				data.Name = types.StringValue(value.String())
			} else {
				data.Name = types.StringNull()
