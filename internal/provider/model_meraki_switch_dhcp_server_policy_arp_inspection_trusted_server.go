...
func (data SwitchDHCPServerPolicyARPInspectionTrustedServer) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("ipv4.address"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ipv4.address", "null")
		} else {
			body, _ = sjson.Set(body, "ipv4.address", value.String())
		}
	}
	return body
}
...
func (data SwitchDHCPServerPolicyARPInspectionTrustedServer) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
