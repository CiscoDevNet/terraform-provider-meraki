...
func (data *ResourceApplianceVLANsIdentity) toIdentity(ctx context.Context, plan *ResourceApplianceVLANs) {
...
func (data *ResourceApplianceVLANs) fromIdentity(ctx context.Context, identity *ResourceApplianceVLANsIdentity) {
...
func (data *ResourceApplianceVLANs) hasChanges(ctx context.Context, state *ResourceApplianceVLANs, id string) bool {
	...
	if len(item.Ipv6PrefixAssignments) != len(stateItem.Ipv6PrefixAssignments) {
		...
		for i := range item.Ipv6PrefixAssignments {
			...
			if !item.Ipv6PrefixAssignments[i].Disabled.Equal(stateItem.Ipv6PrefixAssignments[i].Disabled) {
				hasChanges = true
			}
