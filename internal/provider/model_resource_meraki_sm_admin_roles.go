...
type ResourceSMAdminRoles struct {
	Id             types.String                `tfsdk:"id"`
	OrganizationId types.String                `tfsdk:"organization_id"`
	Items          []ResourceSMAdminRolesItems `tfsdk:"items"`
}

type ResourceSMAdminRolesItems struct {
	Id    types.String `tfsdk:"id"`
...
func (data ResourceSMAdminRoles) getPath() string {
...
func (data ResourceSMAdminRolesItems) toBody(ctx context.Context, state ResourceSMAdminRolesItems) string {
	body := ""
	if !data.Name.IsNull() {
...
func (data *ResourceSMAdminRoles) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]ResourceSMAdminRolesItems, 0)
	if res.Get("items").Exists() {
		res = meraki.Res{Result: res.Get("items")}
	}
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := ResourceSMAdminRolesItems{}
	...
	index := 0
	res.ForEach(func(k, res gjson.Result) bool {
		data.Items[index].Id = types.StringValue(res.Get("roleId").String())
		index++
		return true
	})
...
func (data *ResourceSMAdminRoles) fromBodyPartial(ctx context.Context, res meraki.Res) {
	if res.Get("items").Exists() {
		res = meraki.Res{Result: res.Get("items")}
	}
...
func (data *ResourceSMAdminRoles) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data *ResourceSMAdminRoles) hasChanges(ctx context.Context, state *ResourceSMAdminRoles, id string) bool {
	...
	stateItem := ResourceSMAdminRolesItems{}
	...
	if !item.Scope.Equal(stateItem.Scope) {
		hasChanges = true
	}
