...
func (data NetworkSyslogServers) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("servers"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "servers", "null")
		} else {
			body, _ = sjson.Set(body, "servers", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("roles"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "roles", "null")
					} else {
						var values []string
						for _, v := range value.Array() {
							values = append(values, v.String())
						}
						body, _ = sjson.Set(body, "roles", values)
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "servers.-1", body)
				return true
			})
		}
	}
