...
func TestAccMerakiApplianceInboundFirewallRules(t *testing.T) {
	...
	checks = append(checks, resource.TestCheckResourceAttr("meraki_appliance_inbound_firewall_rules.test", "rules.0.src_cidr", "Any"))
	checks = append(checks, resource.TestCheckResourceAttr("meraki_appliance_inbound_firewall_rules.test", "rules.0.src_port", "Any"))
	checks = append(checks, resource.TestCheckResourceAttr("meraki_appliance_inbound_firewall_rules.test", "rules.0.syslog_enabled", "false"))

	var steps []resource.TestStep
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccMerakiApplianceInboundFirewallRulesPrerequisitesConfig + testAccMerakiApplianceInboundFirewallRulesConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccMerakiApplianceInboundFirewallRulesPrerequisitesConfig + testAccMerakiApplianceInboundFirewallRulesConfig_all(),
		Check:  resource.ComposeTestCheckFunc(checks...),
	})
	steps = append(steps, resource.TestStep{
		ResourceName:            "meraki_appliance_inbound_firewall_rules.test",
		ImportState:             true,
		ImportStateVerify:       true,
		ImportStateIdFunc:       merakiApplianceInboundFirewallRulesImportStateIdFunc("meraki_appliance_inbound_firewall_rules.test"),
...
func merakiApplianceInboundFirewallRulesImportStateIdFunc(resourceName string) resource.ImportStateIdFunc {
...
const testAccMerakiApplianceInboundFirewallRulesPrerequisitesConfig = `
variable "test_org" {}
