...
var (
	_ datasource.DataSource              = &WirelessSSIDOpenRoamingDataSource{}
	_ datasource.DataSourceWithConfigure = &WirelessSSIDOpenRoamingDataSource{}
)

func NewWirelessSSIDOpenRoamingDataSource() datasource.DataSource {
	return &WirelessSSIDOpenRoamingDataSource{}
}

type WirelessSSIDOpenRoamingDataSource struct {
	client *meraki.Client
}

func (d *WirelessSSIDOpenRoamingDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"enabled": schema.BoolAttribute{
				MarkdownDescription: "If true, OpenRoaming is enabled on this SSID.",
				Computed:            true,
			},
			"tenant_id": schema.StringAttribute{
				MarkdownDescription: "The OpenRoaming DNA Spaces tenant ID.",
				Computed:            true,
			},
		},
	}
...
func (d *WirelessSSIDOpenRoamingDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (d *WirelessSSIDOpenRoamingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	settingsPath := fmt.Sprintf("/organizations/%v/wireless/ssids/openRoaming/byNetwork?includeDisabledSsidsboolean=true&networkIds[]=%v", orgId, config.NetworkId.ValueString())
	...
	if len(res.Get("items.0.ssids").Array()) > 0 {
		res.Get("items.0.ssids").ForEach(func(k, v gjson.Result) bool {
			if config.Number.ValueString() == v.Get("number").String() {
