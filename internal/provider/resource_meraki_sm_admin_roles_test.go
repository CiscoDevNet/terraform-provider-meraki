...
func merakiSMAdminRolesImportStateIdFunc(resourceName string) resource.ImportStateIdFunc {
...
const testAccMerakiSMAdminRolesPrerequisitesConfig = `
variable "test_org" {}
...
func testAccMerakiSMAdminRolesConfig_minimum() string {
	config := `resource "meraki_sm_admin_roles" "test" {` + "\n"
...
func testAccMerakiSMAdminRolesConfig_all() string {
	config := `resource "meraki_sm_admin_roles" "test" {` + "\n"
	config += ` organization_id = data.meraki_organization.test.id` + "\n"
	config += ` items = [{` + "\n"
	config += `  name = "sample name"` + "\n"
