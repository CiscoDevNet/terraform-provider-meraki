...
func (data NetworkWebhookHTTPServer) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("url"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "url", "null")
		} else {
			body, _ = sjson.Set(body, "url", value.String())
		}
	}
	if value := res.Get("payloadTemplate.name"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "payloadTemplate.name", "null")
		} else {
			body, _ = sjson.Set(body, "payloadTemplate.name", value.String())
		}
	}
	if value := res.Get("payloadTemplate.payloadTemplateId"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "payloadTemplate.payloadTemplateId", "null")
		} else {
			body, _ = sjson.Set(body, "payloadTemplate.payloadTemplateId", value.String())
		}
	}
	return body
}
...
func (data NetworkWebhookHTTPServer) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
