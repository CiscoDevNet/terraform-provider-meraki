...
type OrganizationInventoryDevices struct {
	OrganizationId types.String                        `tfsdk:"organization_id"`
	Items          []OrganizationInventoryDevicesItems `tfsdk:"items"`
}

type OrganizationInventoryDevicesItems struct {
	...
	Details               []OrganizationInventoryDevicesDetails `tfsdk:"details"`
	Tags                  types.List                            `tfsdk:"tags"`
}

type OrganizationInventoryDevicesDetails struct {
	Name  types.String `tfsdk:"name"`
...
func (data OrganizationInventoryDevices) getPath() string {
...
func (data *OrganizationInventoryDevices) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]OrganizationInventoryDevicesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := OrganizationInventoryDevicesItems{}
		...
		if value := res.Get("details"); value.Exists() && value.Value() != nil {
			data.Details = make([]OrganizationInventoryDevicesDetails, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := OrganizationInventoryDevicesDetails{}
				if value := res.Get("name"); value.Exists() && value.Value() != nil {
					data.Name = types.StringValue(value.String())
				} else {
					data.Name = types.StringNull()
