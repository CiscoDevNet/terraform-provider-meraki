...
func (data WirelessSSIDOpenRoaming) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("tenantId"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "tenantId", "null")
		} else {
			body, _ = sjson.Set(body, "tenantId", value.String())
		}
	}
	return body
}
