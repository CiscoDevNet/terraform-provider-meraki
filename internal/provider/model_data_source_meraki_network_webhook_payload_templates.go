...
type DataSourceNetworkWebhookPayloadTemplates struct {
	NetworkId types.String                                    `tfsdk:"network_id"`
	Items     []DataSourceNetworkWebhookPayloadTemplatesItems `tfsdk:"items"`
}

type DataSourceNetworkWebhookPayloadTemplatesItems struct {
	Id          types.String                                      `tfsdk:"id"`
	Body        types.String                                      `tfsdk:"body"`
	BodyFile    types.String                                      `tfsdk:"body_file"`
	HeadersFile types.String                                      `tfsdk:"headers_file"`
	Name        types.String                                      `tfsdk:"name"`
	Headers     []DataSourceNetworkWebhookPayloadTemplatesHeaders `tfsdk:"headers"`
}

type DataSourceNetworkWebhookPayloadTemplatesHeaders struct {
	Name     types.String `tfsdk:"name"`
...
func (data DataSourceNetworkWebhookPayloadTemplates) getPath() string {
...
func (data *DataSourceNetworkWebhookPayloadTemplates) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]DataSourceNetworkWebhookPayloadTemplatesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := DataSourceNetworkWebhookPayloadTemplatesItems{}
		...
		if value := res.Get("headers"); value.Exists() && value.Value() != nil {
			data.Headers = make([]DataSourceNetworkWebhookPayloadTemplatesHeaders, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := DataSourceNetworkWebhookPayloadTemplatesHeaders{}
				if value := res.Get("name"); value.Exists() && value.Value() != nil {
					data.Name = types.StringValue(value.String())
				} else {
					data.Name = types.StringNull()
