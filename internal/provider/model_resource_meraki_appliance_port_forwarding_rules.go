...
type AppliancePortForwardingRulesIdentity struct {
	NetworkId types.String `tfsdk:"network_id"`
}
...
func (data AppliancePortForwardingRules) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("rules"); value.Exists() {
		if value.Value() == nil {
			...
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("localPort"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "localPort", "null")
					} else {
						body, _ = sjson.Set(body, "localPort", value.String())
					}
				}
				...
				if value := res.Get("publicPort"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "publicPort", "null")
					} else {
						body, _ = sjson.Set(body, "publicPort", value.String())
					}
				}
...
func (data *AppliancePortForwardingRules) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data *AppliancePortForwardingRulesIdentity) toIdentity(ctx context.Context, plan *AppliancePortForwardingRules) {
...
func (data *AppliancePortForwardingRules) fromIdentity(ctx context.Context, identity *AppliancePortForwardingRulesIdentity) {
...
func (data AppliancePortForwardingRules) toDestroyBody(ctx context.Context) string {
	body := ""
	body, _ = sjson.Set(body, "rules", []interface{}{})
	return body
}
