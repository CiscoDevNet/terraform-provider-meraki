...
func (data OrganizationSAMLRole) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("role"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "role", "null")
		} else {
			body, _ = sjson.Set(body, "role", value.String())
		}
	}
