...
// It emits a separate model - DataSourceWirelessSSIDL7FirewallRules - used only by the data source, always including every
...
type DataSourceWirelessSSIDL7FirewallRules struct {
	Id        types.String                                 `tfsdk:"id"`
	NetworkId types.String                                 `tfsdk:"network_id"`
	Number    types.String                                 `tfsdk:"number"`
	Rules     []DataSourceWirelessSSIDL7FirewallRulesRules `tfsdk:"rules"`
}

type DataSourceWirelessSSIDL7FirewallRulesRules struct {
...
func (data DataSourceWirelessSSIDL7FirewallRules) getPath() string {
...
// `type` discriminates whether `value` is read from the raw `value` path or `value.id`.
func (data *DataSourceWirelessSSIDL7FirewallRules) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceWirelessSSIDL7FirewallRulesRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceWirelessSSIDL7FirewallRulesRules{}
