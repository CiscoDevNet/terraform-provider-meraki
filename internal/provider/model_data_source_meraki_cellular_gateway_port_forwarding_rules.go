...
// It emits a separate model - DataSourceCellularGatewayPortForwardingRules - used only by the data source, always including every
...
type DataSourceCellularGatewayPortForwardingRules struct {
	Id     types.String                                        `tfsdk:"id"`
	Serial types.String                                        `tfsdk:"serial"`
	Rules  []DataSourceCellularGatewayPortForwardingRulesRules `tfsdk:"rules"`
}

type DataSourceCellularGatewayPortForwardingRulesRules struct {
...
func (data DataSourceCellularGatewayPortForwardingRules) getPath() string {
...
func (data *DataSourceCellularGatewayPortForwardingRules) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceCellularGatewayPortForwardingRulesRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceCellularGatewayPortForwardingRulesRules{}
