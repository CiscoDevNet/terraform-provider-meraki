...
func (data WirelessSSIDL3FirewallRules) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("rules"); value.Exists() {
		if value.Value() == nil {
			...
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("ipVer"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "ipVer", "null")
					} else {
						body, _ = sjson.Set(body, "ipVer", value.String())
					}
				}
...
func (data *WirelessSSIDL3FirewallRules) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
