...
type DataSourceOrganizationAdmins struct {
	OrganizationId types.String                        `tfsdk:"organization_id"`
	Items          []DataSourceOrganizationAdminsItems `tfsdk:"items"`
}

type DataSourceOrganizationAdminsItems struct {
	Id                   types.String                           `tfsdk:"id"`
	AuthenticationMethod types.String                           `tfsdk:"authentication_method"`
	Email                types.String                           `tfsdk:"email"`
	Name                 types.String                           `tfsdk:"name"`
	OrgAccess            types.String                           `tfsdk:"org_access"`
	Networks             []DataSourceOrganizationAdminsNetworks `tfsdk:"networks"`
	Tags                 []DataSourceOrganizationAdminsTags     `tfsdk:"tags"`
}

type DataSourceOrganizationAdminsNetworks struct {
	Access types.String `tfsdk:"access"`
	Id     types.String `tfsdk:"id"`
}

type DataSourceOrganizationAdminsTags struct {
	Access types.String `tfsdk:"access"`
...
func (data DataSourceOrganizationAdmins) getPath() string {
...
func (data *DataSourceOrganizationAdmins) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]DataSourceOrganizationAdminsItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := DataSourceOrganizationAdminsItems{}
		...
		if value := res.Get("networks"); value.Exists() && value.Value() != nil {
			data.Networks = make([]DataSourceOrganizationAdminsNetworks, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := DataSourceOrganizationAdminsNetworks{}
		...
		if value := res.Get("tags"); value.Exists() && value.Value() != nil {
			data.Tags = make([]DataSourceOrganizationAdminsTags, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := DataSourceOrganizationAdminsTags{}
				if value := res.Get("access"); value.Exists() && value.Value() != nil {
					data.Access = types.StringValue(value.String())
				} else {
					data.Access = types.StringNull()
