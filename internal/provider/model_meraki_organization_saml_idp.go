...
type OrganizationSAMLIdPIdentity struct {
	OrganizationId types.String `tfsdk:"organization_id"`
	Id             types.String `tfsdk:"id"`
}
...
func (data OrganizationSAMLIdP) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("sloLogoutUrl"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "sloLogoutUrl", "null")
		} else {
			body, _ = sjson.Set(body, "sloLogoutUrl", value.String())
		}
	}
	if value := res.Get("ssoLoginUrl"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ssoLoginUrl", "null")
		} else {
			body, _ = sjson.Set(body, "ssoLoginUrl", value.String())
		}
	}
	if value := res.Get("x509certSha1Fingerprint"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "x509certSha1Fingerprint", "null")
		} else {
			body, _ = sjson.Set(body, "x509certSha1Fingerprint", value.String())
		}
	}
	return body
}
...
func (data *OrganizationSAMLIdPIdentity) toIdentity(ctx context.Context, plan *OrganizationSAMLIdP) {
...
func (data *OrganizationSAMLIdP) fromIdentity(ctx context.Context, identity *OrganizationSAMLIdPIdentity) {
