...
func (data ApplianceThirdPartyVPNPeers) toBody(ctx context.Context, state ApplianceThirdPartyVPNPeers) string {
	body := ""
	{
		...
		for _, item := range data.Peers {
			...
			if len(item.EcmpUplinkConfigs) > 0 {
	...
	if value := res.Get("peers"); value.Exists() {
		if value.Value() == nil {
			...
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("ecmpUplinkConfigs"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "ecmpUplinkConfigs", "null")
					} else {
						body, _ = sjson.Set(body, "ecmpUplinkConfigs", []interface{}{})
						parent := &body
						value.ForEach(func(k, res gjson.Result) bool {
							body := ""
							...
							if value := res.Get("wan"); value.Exists() {
								if value.Value() == nil {
									body, _ = sjson.SetRaw(body, "wan", "null")
								} else {
									body, _ = sjson.Set(body, "wan", value.String())
								}
							}
							if value := res.Get("ebgpNeighbor.neighborIp"); value.Exists() {
								if value.Value() == nil {
							...
							*parent, _ = sjson.SetRaw(*parent, "ecmpUplinkConfigs.-1", body)
							return true
						})
					}
				}
