...
type SensorNetworkRelationships struct {
	NetworkId types.String                      `tfsdk:"network_id"`
	Items     []SensorNetworkRelationshipsItems `tfsdk:"items"`
}

type SensorNetworkRelationshipsItems struct {
	Id                                    types.String                                                      `tfsdk:"id"`
	DeviceName                            types.String                                                      `tfsdk:"device_name"`
	DeviceProductType                     types.String                                                      `tfsdk:"device_product_type"`
	DeviceSerial                          types.String                                                      `tfsdk:"device_serial"`
	RelationshipsLivestreamRelatedDevices []SensorNetworkRelationshipsRelationshipsLivestreamRelatedDevices `tfsdk:"relationships_livestream_related_devices"`
}

type SensorNetworkRelationshipsRelationshipsLivestreamRelatedDevices struct {
...
func (data SensorNetworkRelationships) getPath() string {
...
func (data *SensorNetworkRelationships) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]SensorNetworkRelationshipsItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := SensorNetworkRelationshipsItems{}
		...
		if value := res.Get("relationships.livestream.relatedDevices"); value.Exists() && value.Value() != nil {
			data.RelationshipsLivestreamRelatedDevices = make([]SensorNetworkRelationshipsRelationshipsLivestreamRelatedDevices, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := SensorNetworkRelationshipsRelationshipsLivestreamRelatedDevices{}
