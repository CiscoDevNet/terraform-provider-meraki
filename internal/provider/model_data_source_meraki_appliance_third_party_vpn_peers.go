...
type DataSourceApplianceThirdPartyVPNPeersPeers struct {
	...
	EcmpUplinkConfigs                  []DataSourceApplianceThirdPartyVPNPeersPeersEcmpUplinkConfigs `tfsdk:"ecmp_uplink_configs"`
	NetworkTags                        types.List                                                    `tfsdk:"network_tags"`
	PrivateSubnets                     types.List                                                    `tfsdk:"private_subnets"`
}

type DataSourceApplianceThirdPartyVPNPeersPeersEcmpUplinkConfigs struct {
	Id                     types.String `tfsdk:"id"`
	...
	if value := res.Get("peers"); value.Exists() && value.Value() != nil {
		...
		value.ForEach(func(k, res gjson.Result) bool {
			...
			if value := res.Get("ecmpUplinkConfigs"); value.Exists() && value.Value() != nil {
				data.EcmpUplinkConfigs = make([]DataSourceApplianceThirdPartyVPNPeersPeersEcmpUplinkConfigs, 0)
				value.ForEach(func(k, res gjson.Result) bool {
					parent := &data
					data := DataSourceApplianceThirdPartyVPNPeersPeersEcmpUplinkConfigs{}
