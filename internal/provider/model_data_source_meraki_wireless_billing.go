...
// It emits a separate model - DataSourceWirelessBilling - used only by the data source, always including every
...
type DataSourceWirelessBilling struct {
	Id        types.String                     `tfsdk:"id"`
	NetworkId types.String                     `tfsdk:"network_id"`
	Currency  types.String                     `tfsdk:"currency"`
	Plans     []DataSourceWirelessBillingPlans `tfsdk:"plans"`
}

type DataSourceWirelessBillingPlans struct {
...
func (data DataSourceWirelessBilling) getPath() string {
...
func (data *DataSourceWirelessBilling) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("plans"); value.Exists() && value.Value() != nil {
		data.Plans = make([]DataSourceWirelessBillingPlans, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceWirelessBillingPlans{}
