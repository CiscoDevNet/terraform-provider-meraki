...
func (data *SwitchAlternateManagementInterface) fromBodyPartial(ctx context.Context, res gjson.Result) {
	...
	for i := 0; i < len(data.Switches); i++ {
		keys := [...]string{"alternateManagementIp", "gateway", "serial", "subnetMask"}
		keyValues := [...]string{data.Switches[i].AlternateManagementIp.ValueString(), data.Switches[i].Gateway.ValueString(), data.Switches[i].Serial.ValueString(), data.Switches[i].SubnetMask.ValueString()}
