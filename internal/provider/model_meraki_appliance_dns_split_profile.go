...
func (data ApplianceDNSSplitProfile) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("nameservers.addresses"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "nameservers.addresses", "null")
		} else {
			var values []string
	...
	if value := res.Get("hostnames"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "hostnames", "null")
		} else {
			var values []string
			for _, v := range value.Array() {
				values = append(values, v.String())
