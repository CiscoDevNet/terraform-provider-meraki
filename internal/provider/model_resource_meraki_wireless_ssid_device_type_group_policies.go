...
func (data WirelessSSIDDeviceTypeGroupPolicies) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("deviceTypePolicies"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "deviceTypePolicies", "null")
		} else {
			body, _ = sjson.Set(body, "deviceTypePolicies", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("deviceType"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "deviceType", "null")
					} else {
						body, _ = sjson.Set(body, "deviceType", value.String())
					}
				}
				if value := res.Get("groupPolicyId"); value.Exists() {
					if value.Value() == nil {
						...
						body, _ = sjson.Set(body, "groupPolicyId", value.Int())
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "deviceTypePolicies.-1", body)
				return true
			})
		}
	}
