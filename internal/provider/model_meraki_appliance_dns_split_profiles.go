...
type ApplianceDNSSplitProfiles struct {
	OrganizationId types.String                     `tfsdk:"organization_id"`
	Items          []ApplianceDNSSplitProfilesItems `tfsdk:"items"`
}

type ApplianceDNSSplitProfilesItems struct {
...
func (data ApplianceDNSSplitProfiles) getPath() string {
...
func (data *ApplianceDNSSplitProfiles) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]ApplianceDNSSplitProfilesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := ApplianceDNSSplitProfilesItems{}
