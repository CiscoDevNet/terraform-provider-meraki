...
type InsightMonitoredMediaServerIdentity struct {
	OrganizationId types.String `tfsdk:"organization_id"`
}
...
func (data InsightMonitoredMediaServer) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("bestEffortMonitoringEnabled"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "bestEffortMonitoringEnabled", "null")
		} else {
			body, _ = sjson.Set(body, "bestEffortMonitoringEnabled", value.Bool())
		}
	}
...
func (data *InsightMonitoredMediaServerIdentity) toIdentity(ctx context.Context, plan *InsightMonitoredMediaServer) {
...
func (data *InsightMonitoredMediaServer) fromIdentity(ctx context.Context, identity *InsightMonitoredMediaServerIdentity) {
...
func (data InsightMonitoredMediaServer) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
