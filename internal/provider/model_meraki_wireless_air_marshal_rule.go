...
func (data WirelessAirMarshalRule) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("match.string"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "match.string", "null")
		} else {
			body, _ = sjson.Set(body, "match.string", value.String())
		}
	}
	if value := res.Get("match.type"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "match.type", "null")
		} else {
			body, _ = sjson.Set(body, "match.type", value.String())
		}
	}
	return body
}
