...
// It emits a separate model - DataSourceApplianceFirewallMulticastForwarding - used only by the data source, always including every
...
type DataSourceApplianceFirewallMulticastForwarding struct {
	Id        types.String                                          `tfsdk:"id"`
	NetworkId types.String                                          `tfsdk:"network_id"`
	Rules     []DataSourceApplianceFirewallMulticastForwardingRules `tfsdk:"rules"`
}

type DataSourceApplianceFirewallMulticastForwardingRules struct {
...
func (data DataSourceApplianceFirewallMulticastForwarding) getPath() string {
...
func (data *DataSourceApplianceFirewallMulticastForwarding) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceApplianceFirewallMulticastForwardingRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceApplianceFirewallMulticastForwardingRules{}
