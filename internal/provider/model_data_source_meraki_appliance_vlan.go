...
// It emits a separate model - DataSourceApplianceVLAN - used only by the data source, always including every
...
type DataSourceApplianceVLAN struct {
	...
	FixedIpAssignments     map[string]DataSourceApplianceVLANFixedIpAssignments `tfsdk:"fixed_ip_assignments"`
	Ipv6Enabled            types.Bool                                           `tfsdk:"ipv6_enabled"`
	Ipv6PrefixAssignments  []DataSourceApplianceVLANIpv6PrefixAssignments       `tfsdk:"ipv6_prefix_assignments"`
	MandatoryDhcpEnabled   types.Bool                                           `tfsdk:"mandatory_dhcp_enabled"`
	DhcpOptions            []DataSourceApplianceVLANDhcpOptions                 `tfsdk:"dhcp_options"`
	DhcpRelayServerIps     types.List                                           `tfsdk:"dhcp_relay_server_ips"`
	ReservedIpRanges       []DataSourceApplianceVLANReservedIpRanges            `tfsdk:"reserved_ip_ranges"`
}

type DataSourceApplianceVLANFixedIpAssignments struct {
	Ip   types.String `tfsdk:"ip"`
	Name types.String `tfsdk:"name"`
}

type DataSourceApplianceVLANIpv6PrefixAssignments struct {
...
type DataSourceApplianceVLANDhcpOptions struct {
	Code  types.String `tfsdk:"code"`
	Type  types.String `tfsdk:"type"`
	Value types.String `tfsdk:"value"`
}

type DataSourceApplianceVLANReservedIpRanges struct {
...
func (data DataSourceApplianceVLAN) getPath() string {
...
func (data *DataSourceApplianceVLAN) fromBody(ctx context.Context, res meraki.Res) {
	...
	if value := res.Get("fixedIpAssignments"); value.Exists() && value.Value() != nil {
		data.FixedIpAssignments = make(map[string]DataSourceApplianceVLANFixedIpAssignments)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceApplianceVLANFixedIpAssignments{}
	...
	if value := res.Get("ipv6.prefixAssignments"); value.Exists() && value.Value() != nil {
		data.Ipv6PrefixAssignments = make([]DataSourceApplianceVLANIpv6PrefixAssignments, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceApplianceVLANIpv6PrefixAssignments{}
	...
	if value := res.Get("dhcpOptions"); value.Exists() && value.Value() != nil {
		data.DhcpOptions = make([]DataSourceApplianceVLANDhcpOptions, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceApplianceVLANDhcpOptions{}
	...
	if value := res.Get("reservedIpRanges"); value.Exists() && value.Value() != nil {
		data.ReservedIpRanges = make([]DataSourceApplianceVLANReservedIpRanges, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceApplianceVLANReservedIpRanges{}
