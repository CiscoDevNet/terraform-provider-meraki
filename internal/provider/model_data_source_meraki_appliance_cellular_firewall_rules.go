...
// It emits a separate model - DataSourceApplianceCellularFirewallRules - used only by the data source, always including every
...
type DataSourceApplianceCellularFirewallRules struct {
	Id        types.String                                    `tfsdk:"id"`
	NetworkId types.String                                    `tfsdk:"network_id"`
	Rules     []DataSourceApplianceCellularFirewallRulesRules `tfsdk:"rules"`
}

type DataSourceApplianceCellularFirewallRulesRules struct {
...
func (data DataSourceApplianceCellularFirewallRules) getPath() string {
...
func (data *DataSourceApplianceCellularFirewallRules) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("rules"); value.Exists() && value.Value() != nil {
		data.Rules = make([]DataSourceApplianceCellularFirewallRulesRules, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			if value := res.Get("comment"); value.String() == "Default rule" {
				return true
			}
			parent := &data
			data := DataSourceApplianceCellularFirewallRulesRules{}
