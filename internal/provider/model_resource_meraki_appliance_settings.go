...
func (data ApplianceSettings) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("clientTrackingMethod"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "clientTrackingMethod", "null")
		} else {
			body, _ = sjson.Set(body, "clientTrackingMethod", value.String())
		}
	}
	if value := res.Get("deploymentMode"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "deploymentMode", "null")
		} else {
			body, _ = sjson.Set(body, "deploymentMode", value.String())
		}
	}
	if value := res.Get("dynamicDns.enabled"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "dynamicDns.enabled", "null")
		} else {
			body, _ = sjson.Set(body, "dynamicDns.enabled", value.Bool())
		}
	}
	if value := res.Get("dynamicDns.prefix"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "dynamicDns.prefix", "null")
		} else {
			body, _ = sjson.Set(body, "dynamicDns.prefix", value.String())
		}
	}
	return body
}
