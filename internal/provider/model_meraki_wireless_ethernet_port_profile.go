...
type WirelessEthernetPortProfileIdentity struct {
	NetworkId types.String `tfsdk:"network_id"`
	Id        types.String `tfsdk:"id"`
}
...
func (data WirelessEthernetPortProfile) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("ports"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "ports", "null")
		} else {
			body, _ = sjson.Set(body, "ports", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("pskGroupId"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "pskGroupId", "null")
					} else {
						body, _ = sjson.Set(body, "pskGroupId", value.String())
					}
				}
				if value := res.Get("ssid"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "ssid", "null")
					} else {
						body, _ = sjson.Set(body, "ssid", value.Int())
					}
				}
				*parent, _ = sjson.SetRaw(*parent, "ports.-1", body)
				return true
			})
		}
	}
	if value := res.Get("usbPorts"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "usbPorts", "null")
		} else {
			body, _ = sjson.Set(body, "usbPorts", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "usbPorts.-1", body)
				return true
			})
		}
	}
...
func (data *WirelessEthernetPortProfile) fromBodyUnknowns(ctx context.Context, res meraki.Res) {
}
...
func (data *WirelessEthernetPortProfileIdentity) toIdentity(ctx context.Context, plan *WirelessEthernetPortProfile) {
...
func (data *WirelessEthernetPortProfile) fromIdentity(ctx context.Context, identity *WirelessEthernetPortProfileIdentity) {
...
func (data WirelessEthernetPortProfile) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
