...
// It emits a separate model - DataSourceCellularGatewayConnectivityMonitoringDestinations - used only by the data source, always including every
...
type DataSourceCellularGatewayConnectivityMonitoringDestinations struct {
	Id           types.String                                                              `tfsdk:"id"`
	NetworkId    types.String                                                              `tfsdk:"network_id"`
	Destinations []DataSourceCellularGatewayConnectivityMonitoringDestinationsDestinations `tfsdk:"destinations"`
}

type DataSourceCellularGatewayConnectivityMonitoringDestinationsDestinations struct {
...
func (data DataSourceCellularGatewayConnectivityMonitoringDestinations) getPath() string {
...
func (data *DataSourceCellularGatewayConnectivityMonitoringDestinations) fromBody(ctx context.Context, res meraki.Res) {
	if value := res.Get("destinations"); value.Exists() && value.Value() != nil {
		data.Destinations = make([]DataSourceCellularGatewayConnectivityMonitoringDestinationsDestinations, 0)
		value.ForEach(func(k, res gjson.Result) bool {
			parent := &data
			data := DataSourceCellularGatewayConnectivityMonitoringDestinationsDestinations{}
