...
func (data OrganizationLicense) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("deviceSerial"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "deviceSerial", "null")
		} else {
			body, _ = sjson.Set(body, "deviceSerial", value.String())
		}
	}
	return body
}
