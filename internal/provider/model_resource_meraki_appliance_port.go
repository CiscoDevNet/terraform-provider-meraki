...
func (data AppliancePort) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("accessPolicy"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "accessPolicy", "null")
		} else {
			body, _ = sjson.Set(body, "accessPolicy", value.String())
		}
	}
	...
	if value := res.Get("dropUntaggedTraffic"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "dropUntaggedTraffic", "null")
		} else {
			body, _ = sjson.Set(body, "dropUntaggedTraffic", value.Bool())
		}
	}
