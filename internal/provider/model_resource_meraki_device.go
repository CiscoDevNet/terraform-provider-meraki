...
func (data Device) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("lat"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "lat", "null")
		} else {
			body, _ = sjson.Set(body, "lat", value.Float())
		}
	}
	if value := res.Get("lng"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "lng", "null")
		} else {
			body, _ = sjson.Set(body, "lng", value.Float())
		}
	}
...
func (data *DeviceIdentity) toIdentity(ctx context.Context, plan *Device) {
...
func (data *Device) fromIdentity(ctx context.Context, identity *DeviceIdentity) {
...
func (data Device) toDestroyBody(ctx context.Context) string {
	body := ""
	return body
}
