...
func (data ApplianceOneToManyNATRules) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("rules"); value.Exists() {
		if value.Value() == nil {
			...
			value.ForEach(func(k, res gjson.Result) bool {
				...
				if value := res.Get("portRules"); value.Exists() {
					if value.Value() == nil {
						body, _ = sjson.SetRaw(body, "portRules", "null")
					} else {
						body, _ = sjson.Set(body, "portRules", []interface{}{})
						parent := &body
						value.ForEach(func(k, res gjson.Result) bool {
							body := ""
							if value := res.Get("localIp"); value.Exists() {
								if value.Value() == nil {
									body, _ = sjson.SetRaw(body, "localIp", "null")
								} else {
									body, _ = sjson.Set(body, "localIp", value.String())
								}
							}
							...
							*parent, _ = sjson.SetRaw(*parent, "portRules.-1", body)
							return true
						})
					}
				}
