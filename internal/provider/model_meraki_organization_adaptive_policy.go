...
func (data OrganizationAdaptivePolicy) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	if value := res.Get("lastEntryRule"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "lastEntryRule", "null")
		} else {
			body, _ = sjson.Set(body, "lastEntryRule", value.String())
		}
	}
	if value := res.Get("destinationGroup.id"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "destinationGroup.id", "null")
		} else {
			body, _ = sjson.Set(body, "destinationGroup.id", value.String())
		}
	}
	if value := res.Get("destinationGroup.name"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "destinationGroup.name", "null")
		} else {
			body, _ = sjson.Set(body, "destinationGroup.name", value.String())
		}
	}
	if value := res.Get("destinationGroup.sgt"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "destinationGroup.sgt", "null")
		} else {
			body, _ = sjson.Set(body, "destinationGroup.sgt", value.Int())
		}
	}
	if value := res.Get("sourceGroup.id"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "sourceGroup.id", "null")
		} else {
			body, _ = sjson.Set(body, "sourceGroup.id", value.String())
		}
	}
	if value := res.Get("sourceGroup.name"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "sourceGroup.name", "null")
		} else {
			body, _ = sjson.Set(body, "sourceGroup.name", value.String())
		}
	}
	if value := res.Get("sourceGroup.sgt"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "sourceGroup.sgt", "null")
		} else {
			body, _ = sjson.Set(body, "sourceGroup.sgt", value.Int())
		}
	}
	if value := res.Get("acls"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "acls", "null")
		} else {
			body, _ = sjson.Set(body, "acls", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "acls.-1", body)
				return true
			})
		}
	}
