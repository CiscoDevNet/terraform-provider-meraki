...
func (d *NetworkGroupPoliciesDataSource) ConfigValidators(ctx context.Context) []datasource.ConfigValidator {
	return []datasource.ConfigValidator{
		datasourcevalidator.ExactlyOneOf(
			path.MatchRoot("group_policy_id"),
			path.MatchRoot("name"),
		),
	}
}
...
func (d *NetworkGroupPoliciesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	tflog.Debug(ctx, fmt.Sprintf("%s: Beginning Read", config.GroupPolicyId.String()))

	var res gjson.Result
	var err error
	if config.GroupPolicyId.IsNull() && !config.Name.IsNull() {
		...
		if len(res.Array()) > 0 {
			res.ForEach(func(k, v gjson.Result) bool {
				if config.Name.ValueString() == v.Get("name").String() {
					config.GroupPolicyId = types.StringValue(v.Get("groupPolicyId").String())
					tflog.Debug(ctx, fmt.Sprintf("%s: Found object with name '%v', id: %v", config.GroupPolicyId.String(), config.Name.ValueString(), config.GroupPolicyId.String()))
					res = v
					return false
				}
				return true
		...
		if config.GroupPolicyId.IsNull() {
	...
	if !res.Exists() {
		res, err = d.client.Get(config.getPath() + "/" + url.QueryEscape(config.GroupPolicyId.ValueString()))
		if err != nil {
	...
	tflog.Debug(ctx, fmt.Sprintf("%s: Read finished successfully", config.GroupPolicyId.ValueString()))
