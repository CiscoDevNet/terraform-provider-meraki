...
type DataSourceOrganizationLicenses struct {
	OrganizationId types.String                          `tfsdk:"organization_id"`
	Items          []DataSourceOrganizationLicensesItems `tfsdk:"items"`
}

type DataSourceOrganizationLicensesItems struct {
	...
	PermanentlyQueuedLicenses []DataSourceOrganizationLicensesPermanentlyQueuedLicenses `tfsdk:"permanently_queued_licenses"`
}

type DataSourceOrganizationLicensesPermanentlyQueuedLicenses struct {
	DurationInDays types.Int64  `tfsdk:"duration_in_days"`
	Id             types.String `tfsdk:"id"`
	LicenseKey     types.String `tfsdk:"license_key"`
	LicenseType    types.String `tfsdk:"license_type"`
...
func (data DataSourceOrganizationLicenses) getPath() string {
...
func (data *DataSourceOrganizationLicenses) fromBody(ctx context.Context, res meraki.Res) {
	data.Items = make([]DataSourceOrganizationLicensesItems, 0)
	res.ForEach(func(k, res gjson.Result) bool {
		parent := &data
		data := DataSourceOrganizationLicensesItems{}
		...
		if value := res.Get("permanentlyQueuedLicenses"); value.Exists() && value.Value() != nil {
			data.PermanentlyQueuedLicenses = make([]DataSourceOrganizationLicensesPermanentlyQueuedLicenses, 0)
			value.ForEach(func(k, res gjson.Result) bool {
				parent := &data
				data := DataSourceOrganizationLicensesPermanentlyQueuedLicenses{}
				if value := res.Get("durationInDays"); value.Exists() && value.Value() != nil {
					data.DurationInDays = types.Int64Value(value.Int())
				} else {
					data.DurationInDays = types.Int64Null()
