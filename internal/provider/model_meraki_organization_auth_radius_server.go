...
func (data OrganizationAuthRADIUSServer) toBodyPreservingNulls(ctx context.Context, res meraki.Res) string {
	body := ""
	...
	if value := res.Get("modes"); value.Exists() {
		if value.Value() == nil {
			body, _ = sjson.SetRaw(body, "modes", "null")
		} else {
			body, _ = sjson.Set(body, "modes", []interface{}{})
			parent := &body
			value.ForEach(func(k, res gjson.Result) bool {
				...
				*parent, _ = sjson.SetRaw(*parent, "modes.-1", body)
				return true
			})
		}
	}
