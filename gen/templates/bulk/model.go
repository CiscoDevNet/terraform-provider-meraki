...
func (data {{camelCase .BulkName}}) toBody(ctx context.Context, state {{camelCase .BulkName}}) string {
	body := "[]"
	if len(data.Items) > 0 {
		for _, item := range data.Items {
			...
			{{- range .Attributes}}
			{{- if .Value}}
			iBody, _ = sjson.Set(iBody, "{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}", {{if eq .Type "String"}}"{{end}}{{.Value}}{{if eq .Type "String"}}"{{end}})
			...
			if !item.{{toGoName .TfName}}.IsNull() {{if .WriteChangesOnly}}&& item.{{toGoName .TfName}} != state.{{toGoName .TfName}}{{end}} {
				iBody, _ = sjson.Set(iBody, "{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}", item.{{toGoName .TfName}}.Value{{.Type}}())
			}
			{{- else if isListSet .}}
			if !item.{{toGoName .TfName}}.IsNull() {
				...
				iBody, _ = sjson.Set(iBody, "{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}", values)
			}
			{{- else if isNestedListSet .}}
			{{if not .Mandatory}}if len(item.{{toGoName .TfName}}) > 0 {{end}}{
				iBody, _ = sjson.Set(iBody, "{{range .DataPath}}{{.}}.{{end}}{{.ModelName}}", []interface{}{})
				for _, item := range item.{{toGoName .TfName}} {
					...
					{{- end}}
					{{- end}}
					iBody, _ = sjson.SetRaw(iBody, "{{range .DataPath}}{{.}}.{{end}}{{if .ModelName}}{{.ModelName}}.{{end}}-1", itemBody)
				}
			}
			{{- end}}
			{{- end}}
			{{- end}}
			body, _ = sjson.SetRaw(body, ".-1", iBody)
		}
	}
	return body
}
